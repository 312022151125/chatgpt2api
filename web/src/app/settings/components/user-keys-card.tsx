...
export function UserKeysCard() {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      setEditingItem(null);
      setEditKey("");
      toast.success(trimmedKey ? "User key updated" : "Name updated");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to update user key");
    } finally {
      setItemPending(item.id, false);
        ...
        <CardContent className="space-y-6 p-6">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-stone-500">Create dedicated keys for regular users; regular users can only access the drawing page and cannot view settings or the account pool.</p>
              </div>
            </div>
          ...
          {revealedKey ? (
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-4 text-sm text-emerald-900">
              <div className="font-medium">The new key is shown only once. Please save it immediately:</div>
          ...
          {isLoading ? (
            ...
            <div className="rounded-xl bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">
              No regular user keys yet. Click the button in the top-right corner to create and share one.
            </div>
          ) : (
        ...
        <DialogContent className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            ...
            <DialogDescription className="text-sm leading-6">
              Optionally enter a note name to distinguish different users. A raw key that can only be viewed once will be generated after creation.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="e.g. Designer A, Temp Account"
      ...
      <Dialog open={Boolean(deletingItem)} onOpenChange={(open) => (!open ? setDeletingItem(null) : null)}>
        <DialogContent className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            ...
            <DialogDescription className="text-sm leading-6">
              Are you sure you want to delete the user key &quot;{deletingItem?.name}&quot;? After deletion, this key will no longer be able to call APIs.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
      ...
      >
        <DialogContent className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            ...
            <DialogDescription className="text-sm leading-6">
              You can update the note name. To rotate the dedicated key, simply enter a new raw key. Leave it blank to keep the current key.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs leading-5 text-stone-500">
                The old key will be invalidated and the new key will take effect immediately after saving. The system still only stores a hash and will not echo the current key.
              </p>
            </div>
          </div>
          <DialogFooter>
