        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-stone-500">Configure a unified proxy for outbound requests. Changes take effect immediately after saving.</p>
            </div>
          </div>
        ...
        {isLoadingConfig ? (
          ...
          <>
            <div className="space-y-2">
              <label className="text-sm font-medium text-stone-700">Proxy URL</label>
              <Input
                value={proxy}
                onChange={(event) => {
              ...
              <p className="text-sm text-stone-500">
                Leave blank to disable the proxy. Enter the full URL, e.g. `http://127.0.0.1:7890`, `http://user:pass@127.0.0.1:7890`, or `socks5://127.0.0.1:7890`.
              </p>
            </div>
            ...
            {testResult ? (
              ...
              >
                {testResult.ok
                  ? `Proxy available: HTTP ${testResult.status}, ${testResult.latency_ms} ms`
                  : `Proxy unavailable: ${testResult.error ?? "Unknown error"} (${testResult.latency_ms} ms)`}
              </div>
            ) : null}
