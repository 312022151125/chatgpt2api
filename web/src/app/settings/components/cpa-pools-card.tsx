      ...
      <CardContent className="space-y-6 p-6">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-stone-500">Configure a connection first, then query remote accounts on demand and import them into the local pool.</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {pools.length > 0 ? <Badge className="rounded-md px-2.5 py-1">{pools.length} connection{pools.length === 1 ? "" : "s"}</Badge> : null}
        ...
        <div className="rounded-xl bg-stone-50 px-4 py-3 text-sm leading-6 text-stone-500">
          ...
          <ul className="mt-1 list-inside list-disc space-y-0.5">
            <li>When the page loads, it reads the CPA connections already configured in the system.</li>
            <li>Click Sync on a connection to fetch the remote account list and present it for selection.</li>
            <li>After confirming your selection, the backend downloads the corresponding access_token and imports it into the local account pool in the background.</li>
            <li>The frontend only polls the import progress and does not directly participate in the download.</li>
          </ul>
        </div>
      </CardContent>
    </Card>
