...
export function ProxyRuntimeCard() {
  ...
  const handleTestRuntimeProxy = async () => {
    ...
    try {
      ...
      if (data.result.ok) {
        toast.success(`Clearance proxy available (${data.result.latency_ms} ms, HTTP ${data.result.status})`);
      } else {
        toast.error(`Clearance proxy unavailable: ${data.result.error ?? "Unknown error"}`);
      }
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to test clearance proxy");
    } finally {
  ...
  const handleTestClearance = async () => {
    ...
    try {
      ...
      if (data.result.ok) {
        toast.success(`Clearance acquired successfully (${data.result.latency_ms} ms)`);
      } else {
    ...
    <Card className="rounded-2xl border-white/80 bg-white/90 shadow-sm">
      <CardContent className="space-y-5 p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs leading-6 text-stone-500">
              Disabled by default. Used to obtain clearance after upstream requests encounter Cloudflare interception; can be combined with WARP / Privoxy proxy chains for retries.
            </p>
          </div>
        ...
        <div className="rounded-xl border border-stone-200 bg-stone-50 px-4 py-3 text-xs leading-6 text-stone-600">
          Proxy priority: Account proxy &gt; FlareSolverr proxy chain &gt; Explicit proxy &gt; Global proxy. Cookie / cf_clearance will not be returned in plain text in API responses.
        </div>
        ...
        <div className="flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-6 text-amber-800">
          ...
          <span>Before using FlareSolverr mode, start the flaresolverr, privoxy, warp-proxy, and related containers via Docker; the in-container URL is usually http://flaresolverr:8191.</span>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs leading-5 text-stone-500">
              Supports http/https/socks5/socks5h; socks5 is converted to socks5h. Auth format: protocol://username:password@host:port, or paste host:port:username:password directly.
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Leave empty to reuse clearance proxy"
              className="h-10 rounded-xl border-stone-200 bg-white"
              disabled={!runtimeEnabled || runtime.egress_mode !== "single_proxy"}
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              disabled={!runtimeEnabled}
            />
            <p className="text-xs text-stone-500">Defaults to 403; only triggered for Cloudflare / challenge errors.</p>
          </div>
          ...
          {proxyResult ? (
            <div className={`rounded-xl border px-3 py-2 text-xs leading-6 md:col-span-2 ${proxyResult.ok ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-rose-200 bg-rose-50 text-rose-800"}`}>
              {proxyResult.ok
                ...
                : `Proxy unavailable: ${proxyResult.error ?? "Unknown error"} (time ${proxyResult.latency_ms} ms)`}
            </div>
          ) : null}
        </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <p className="text-xs text-stone-500">
                {hasStoredClearance ? "A Cookie/clearance has been saved on the server; leaving it blank when saving will not clear the existing value." : "Leave blank to not use manual Cookie."}
              </p>
            </div>
            ...
            {clearanceResult ? (
              <div className={`rounded-xl border px-3 py-2 text-xs leading-6 md:col-span-2 ${clearanceResult.ok ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-rose-200 bg-rose-50 text-rose-800"}`}>
                {clearanceResult.ok
                  ? `Clearance available: ${clearanceResult.has_cookies ? "Cookie acquired" : "No Cookie"}, time ${clearanceResult.latency_ms} ms`
                  : `Clearance unavailable: ${clearanceResult.error ?? clearanceResult.status} (time ${clearanceResult.latency_ms} ms)`}
              </div>
            ) : null}
          </div>
        </div>
