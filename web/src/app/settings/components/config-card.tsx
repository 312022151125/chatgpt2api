...
export function ConfigCard() {
  ...
  const handleTestProxy = async () => {
    ...
    if (!candidate) {
      toast.error("Please fill in the proxy address");
      return;
    }
    ...
    <Card className="rounded-2xl border-white/80 bg-white/90 shadow-sm">
      <CardContent className="space-y-4 p-6">
        <div className="rounded-xl border border-stone-200 bg-stone-50 px-4 py-3 text-sm leading-6 text-stone-600">
          The admin login key is still read from deployment configuration and is no longer shown on this page. To share
          it with others, create a regular user key below.
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-stone-500">In minutes; controls how often accounts refresh automatically.</p>
          </div>
          <div className="space-y-2">
            ...
            <p className="text-xs leading-5 text-stone-500">
              Leave blank to not use a proxy. Supports protocol://username:password@host:port, or paste the provider's
              host:port:username:password directly; e.g., http://user:pass@127.0.0.1:7890,
              127.0.0.1:7890:user:pass. URL-encode @/: and other special characters in username/password.
            </p>
            {proxyTestResult ? (
              <div
          ...
          <div className="space-y-2">
            <label className="text-sm text-stone-700">Image base URL</label>
            <Input
              ...
              className="h-10 rounded-xl border-stone-200 bg-white"
            />
            <p className="text-xs text-stone-500">The access prefix used for generated image results.</p>
          </div>
          <div className="space-y-2">
            <label className="text-sm text-stone-700">Auto image cleanup</label>
            <Input
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              className="h-10 rounded-xl border-stone-200 bg-white"
            />
            <p className="text-xs text-stone-500">In seconds; maximum time to wait for upstream image results.</p>
          </div>
          <div className="space-y-2">
            ...
            <p className="text-xs text-stone-500">
              Limit how many image requests each account processes at the same time. Default is 3.
            </p>
          </div>
          <div className="space-y-2">
            ...
            <p className="text-xs text-stone-500">Detect and remove invalid accounts on refresh.</p>
          </div>
          <div className="space-y-2">
            ...
            <p className="text-xs text-stone-500">Slightly improves the success rate of fetching images when enabled.</p>
          </div>
          <div className="space-y-2">
            <div className="flex items-center gap-3 rounded-xl border border-stone-200 bg-white px-4 py-3">
              <Checkbox
            ...
            <p className="text-xs text-stone-500">
              After successfully retrieving the image, asynchronously hide the corresponding local ChatGPT conversation
              record.
            </p>
          </div>
          <div className="space-y-2">
            <label className="text-sm text-stone-700">Extra wait after image timeout</label>
            <Input
            ...
            />
            <p className="text-xs text-stone-500">
              In seconds; extra waiting time after clicking &quot;Continue waiting&quot; when timed out.
            </p>
          </div>
          <div className="space-y-2">
            <label className="text-sm text-stone-700">Image settle wait time</label>
            <Input
            ...
            />
            <p className="text-xs text-stone-500">
              In seconds; how long to wait after finding an image before confirming again. Requires the image settle
              mechanism.
            </p>
          </div>
          <div className="flex gap-4 md:col-span-2">
            <div className="flex-1 space-y-2">
              <label className="flex items-center gap-3 rounded-xl border border-stone-200 bg-white px-4 py-3 text-sm text-stone-700">
                ...
                />
                Auto-relogin after refresh
              </label>
              <p className="text-xs text-stone-500">
                After enabling, automatically try password login to recover accounts during refresh.
              </p>
            </div>
          ...
          <div className="space-y-3 rounded-xl border border-stone-200 bg-white px-4 py-3">
            <div>
              ...
              <p className="mt-1 text-xs text-stone-500">When none selected, defaults to info / warning / error.</p>
            </div>
          ...
          <div className="space-y-2 md:col-span-2">
            ...
            />
            <p className="text-xs text-stone-500">
              Injected as a system message on every request; can be used to review user prompts, avoid non-compliant
              content, unify model behavior, or fix role settings.
            </p>
          </div>
          <div className="space-y-2 md:col-span-2">
            ...
            <Textarea
              ...
              placeholder="One per line; matches are rejected"
              className="min-h-28 rounded-xl border-stone-200 bg-white font-mono text-xs shadow-none"
            />
            <p className="text-xs text-stone-500">Reject immediately if a user request contains any sensitive word.</p>
          </div>
          <div className="space-y-4 rounded-xl border border-stone-200 bg-white px-4 py-3 md:col-span-2">
            ...
            <p className="text-xs leading-6 text-stone-500">
              Only newly generated images are processed during generation; Sync all uploads existing local images to
              WebDAV.
            </p>
            <div className="rounded-lg border border-stone-100 bg-stone-50 px-3 py-2 text-xs text-stone-600">
              ...
              <span className="ml-2 text-stone-400">
                Changes need to be saved, or will be auto-saved via the Test / Sync buttons.
              </span>
            </div>
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <div className="space-y-2 md:col-span-3">
                ...
                />
                <p className="text-xs text-stone-500">
                  When blank, returns this app's /images/... proxy URL; when filled, returns the public image URL
                  directly.
                </p>
              </div>
            </div>
          ...
          <div className="space-y-4 rounded-xl border border-stone-200 bg-white px-4 py-3 md:col-span-2">
            ...
            <p className="text-xs leading-6 text-stone-500">
              When enabled, calls the review model before requests reach the image generation account. Requests that fail
              review are rejected directly, reducing the risk of non-compliant prompts triggering account restrictions
              or bans.
            </p>
            <div className="grid gap-4 md:grid-cols-3">
              <div className="space-y-2">
