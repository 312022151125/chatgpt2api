...
export function Sub2APIConnections() {
  ...
  const handleFetchGroups = async () => {
    if (!editingServer) {
      toast.error("Please save the connection before fetching groups");
      return;
    }
    ...
    try {
      ...
      if (data.groups.length === 0) {
        toast.message("Remote has no groups configured");
      } else {
        toast.success(`Read ${data.groups.length} groups`);
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    if (!formBaseUrl.trim()) {
      toast.error("Please enter Sub2API URL");
      return;
    }
  ...
  const handleStartImport = async () => {
    ...
    if (selectedIds.length === 0) {
      toast.error("Please select accounts to import");
      return;
    }
        ...
        <CardContent className="space-y-6 p-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-stone-500">
                  After configuring a Sub2API server, query its OpenAI OAuth accounts and import them into the local account pool in bulk.
                </p>
              </div>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-3 rounded-xl bg-stone-50 px-6 py-10 text-center">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-stone-400">Click "Add Connection" to save your Sub2API information.</p>
              </div>
            </div>
          ) : (
          ...
          <div className="rounded-xl bg-stone-50 px-4 py-3 text-sm leading-6 text-stone-500">
            ...
            <ul className="mt-1 list-inside list-disc space-y-0.5">
              <li>Enter the Sub2API URL and admin credentials (or Admin API Key), then save as a connection.</li>
              <li>Click "Sync" on a connection to fetch accounts where platform=openai and type=oauth.</li>
              <li>Select the accounts you need; the backend fetches access_token concurrently, imports them into the local account pool, and refreshes their status.</li>
              <li>Only the access_token from the Sub2API credential is read; fields like refresh_token are not written locally.</li>
            </ul>
          </div>
        </CardContent>
      </Card>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      placeholder={editingServer ? "Leave blank to keep password unchanged" : "Admin password"}
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    placeholder={editingServer ? "Leave blank to keep key unchanged" : "Sub2API Admin API Key"}
                    className="h-11 rounded-xl border-stone-200 bg-white pr-10"
                  />
                  <button
                    type="button"
            ...
            <div className="space-y-2">
              ...
              {remoteGroups && remoteGroups.length > 0 ? (
                ...
                <Input
                  ...
                  placeholder="Leave blank to sync all groups; or enter a group ID / ungrouped"
                  className="h-11 rounded-xl border-stone-200 bg-white"
                />
              )}
              {editingServer ? (
                <div className="flex items-center justify-between gap-2 text-xs text-stone-500">
                  <span>Sync filters by group ID; blank = sync all OpenAI OAuth accounts.</span>
                  <Button
                    variant="outline"
                  ...
                  >
                    ...
                    {remoteGroups ? "Refetch" : "Fetch groups"}
                  </Button>
                </div>
              ) : (
                <div className="text-xs text-stone-500">
                  After adding the connection, open the edit dialog and click "Fetch groups" to select a specific group.
                </div>
              )}
            </div>
          </div>
        ...
        <DialogContent showCloseButton={false} className="max-h-[90vh] max-w-5xl rounded-2xl p-6">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {allFilteredSelected ? "Deselect all" : "Select all filtered results"}
              </Button>
            </div>
          </div>
