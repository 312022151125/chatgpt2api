...
const includeLabels: Array<{ key: keyof BackupInclude; label: string }> = [
  ...
  { key: "accounts_snapshot", label: "Account Snapshots" },
  { key: "auth_keys_snapshot", label: "User Key Snapshots" },
      ...
      <Card className="rounded-2xl border-white/80 bg-white/90 shadow-sm">
        <CardContent className="space-y-6 p-6">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-stone-500">Back up critical data to Cloudflare R2 on a schedule, with optional encryption, rotation, manual execution, and history cleanup.</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Badge variant={backupState?.running ? "warning" : backupState?.last_status === "success" ? "success" : "secondary"} className="rounded-md">
                {backupState?.running ? "Running" : backupState?.last_status === "success" ? "Last Success" : backupState?.last_status === "error" ? "Last Error" : "Not Run"}
              </Badge>
            </div>
          </div>

          <div className="rounded-xl border border-stone-200 bg-stone-50 px-4 py-3 text-sm leading-6 text-stone-600">
            Accounts and user keys are exported as logical snapshots from the current storage backend, independent of whether the underlying store is `json`, `sqlite`, `postgres`, or `git`. The images directory is not backed up by default to avoid large backup sizes.
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input value={String(backup.prefix || "")} onChange={(event) => setBackupField("prefix", event.target.value)} className="h-10 rounded-xl border-stone-200 bg-white" />
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-stone-500">In minutes. The service will poll and run automatically at this interval after startup.</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-stone-500">Automatically deletes older backups after a successful upload. Enter `0` to disable automatic rotation.</p>
          </div>
          <div className="space-y-2">
            <label className="text-sm text-stone-700">Encryption Passphrase</label>
            <Input type="password" value={String(backup.passphrase || "")} onChange={(event) => setBackupField("passphrase", event.target.value)} placeholder={backup.encrypt ? "Required when encryption is enabled" : "Leave blank"} className="h-10 rounded-xl border-stone-200 bg-white" />
            <p className="text-xs text-stone-500">Only used when encryption is enabled. Keep it safe, or you will be unable to decrypt the backup contents.</p>
          </div>
          </div>
          ...
          <div>
            <div className="text-xs text-stone-500">Latest Object</div>
          ...
          {isLoadingBackups ? (
            ...
            <div className="rounded-xl bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">
              No remote backups yet. They will appear here after saving the config and running a manual backup.
            </div>
          ) : (
            <div className="space-y-3">
              {backups.map((item) => {
        ...
        <DialogContent className="flex max-h-[85vh] max-w-3xl flex-col overflow-hidden rounded-2xl border-white/80 bg-white">
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
            {detailLoading ? (
              ...
              <div className="rounded-xl bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">
                Unable to read backup details for now. If this is an encrypted backup, please make sure the correct encryption passphrase is entered and save the config first.
              </div>
            ) : (
              <>
