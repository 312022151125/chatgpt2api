...
function ImagePageContent({ isAdmin }: { isAdmin: boolean }) {
  ...
  const deleteConfirmDescription =
    deleteConfirm?.type === "all"
      ...
      : deleteConfirm?.type === "prompt"
        ? "Delete this prompt record? The generated results will be kept."
        : deleteConfirm?.type === "results"
  ...
  const handleContinueEdit = useCallback(
    async (conversationId: string, image: StoredImage | StoredReferenceImage) => {
      try {
        ...
        setImagePrompt("");
        textareaRef.current?.focus();
        toast.success("Added to current reference images. Continue describing to edit.");
      } catch (error) {
  ...
  const handleTimeoutRetryContinue = useCallback(async () => {
    ...
    try {
      ...
      toast.info(`Continued waiting for ${imageTimeoutRetrySecs}s`);
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Polling retry failed";
      toast.error(msg);
      setTimeoutRetry(null);
    }
  ...
  const handleSubmit = async () => {
    ...
    if (targetStats.running > 0 || targetStats.queued > 1) {
      ...
      toast.success("New conversation created and processing");
    } else {
