      ...
      <section className="flex min-h-0 flex-col">
        ...
        <div className="min-h-0 flex-1 overflow-auto p-5">
          {selectedTask ? (
            <div className="space-y-5">
              ...
              {selectedTask.result ? (
                <div className="space-y-3 rounded-md border border-stone-200 bg-white p-4 dark:border-white/10 dark:bg-white/[0.03]">
                  ...
                  <ResultFile href={selectedTask.result.zip_url} icon={<FileArchive className="size-4" />} label="Asset pack" />
                </div>
              ) : null}
            ...
            <div className="flex h-full min-h-80 items-center justify-center text-sm text-stone-400 dark:text-stone-500">No task</div>
          )}
        </div>
      </section>
    </div>
    <Dialog open={!!deleteConfirm} onOpenChange={(open) => !open && setDeleteConfirm(null)}>
      <DialogContent className="rounded-xl" showCloseButton={false}>
        <DialogHeader>
          ...
          <DialogDescription>
            {deleteConfirm?.type === "all" ? "Are you sure you want to clear this history?" : "Are you sure you want to delete this history record?"}
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
