...
function ImageManagerContent() {
  ...
  const handleSetTags = async (item: ManagedImage, tags: string[]) => {
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Failed to set tag");
    }
  };
    ...
    <section className="space-y-5">
        ...
        {storage ? (
          <>
            <div className="rounded-xl border border-stone-200 bg-white/80 p-3">
              <div className="text-xs text-stone-500">Disk total</div>
            ...
            <div className="rounded-xl border border-stone-200 bg-white/80 p-3">
              <div className="text-xs text-stone-500">Image usage</div>
          ...
          <div className="rounded-xl border border-stone-200 bg-white/80 p-3 col-span-full text-center text-sm text-stone-400">
            {storageLoading ? "Loading storage..." : "Failed to load storage"}
          </div>
        )}
      </div>
        ...
        <DialogContent className="sm:max-w-md rounded-2xl">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-stone-400">images before</span>
            </div>
            <p className="text-xs text-stone-500">This cannot be undone. All images matching the date and their thumbnails will be permanently deleted.</p>
          </div>
          <DialogFooter>
      ...
      <Dialog open={dialogVisible} onOpenChange={(open) => { if (!open) closeDialog(); }}>
        <DialogContent className="max-w-sm overflow-hidden rounded-2xl">
          <DialogHeader>
            <DialogTitle className="pr-8">Delete</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={deleteMode === "selected" || deleteMode === "filtered"} onOpenChange={(open) => (!open ? setDeleteMode(null) : null)}>
        <DialogContent showCloseButton={false} className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            <DialogTitle>{deleteMode === "filtered" ? "Delete images by date" : "Delete selected images"}</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-stone-600">
