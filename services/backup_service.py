...
def _openssl_encrypt(data: bytes, passphrase: str) -> bytes:
    ...
    try:
        ...
        raise BackupError("openssl is missing; encrypted backup is unavailable.") from exc
    except subprocess.CalledProcessError as exc:
        detail = (exc.stderr or b"").decode("utf-8", errors="replace").strip()
        raise BackupError(f"Encrypted backup failed: {detail or 'openssl command failed'}") from exc
...
def _openssl_decrypt(data: bytes, passphrase: str) -> bytes:
    ...
    try:
        ...
        raise BackupError("openssl is missing; encrypted backup content cannot be decrypted.") from exc
    except subprocess.CalledProcessError as exc:
        detail = (exc.stderr or b"").decode("utf-8", errors="replace").strip()
        raise BackupError(f"Backup decryption failed: {detail or 'openssl command failed'}") from exc
    return result.stdout
...
class CloudflareR2Client:
    ...
    def validate(self) -> None:
        ...
        if missing:
            raise BackupError(f"R2 configuration is incomplete: missing {', '.join(missing)}")
    ...
    def test_connection(self) -> dict[str, object]:
        ...
        if response.status_code >= 400:
            raise BackupError(f"R2 connection failed: HTTP {response.status_code}")
    ...
    def upload_bytes(self, key: str, payload: bytes, *, content_type: str, metadata: dict[str, str] | None = None) -> dict[str, object]:
        ...
        if response.status_code >= 400:
            raise BackupError(f"Backup upload failed: HTTP {response.status_code}")
    ...
    def delete_object(self, key: str) -> None:
        ...
        if response.status_code >= 400 and response.status_code != 404:
            raise BackupError(f"Backup deletion failed: HTTP {response.status_code}")

    def download_bytes(self, key: str) -> bytes:
        response = self._request("GET", key, timeout=60.0)
        if response.status_code >= 400:
            raise BackupError(f"Backup read failed: HTTP {response.status_code}")
    ...
    def list_objects(self) -> list[dict[str, object]]:
        ...
        while True:
            ...
            if response.status_code >= 400:
                raise BackupError(f"Failed to list backups: HTTP {response.status_code}")
            text = response.text
...
class BackupService:
    ...
    def download_backup(self, key: str) -> dict[str, object]:
        ...
        if candidate.endswith(".enc"):
            ...
            if not passphrase:
                raise BackupError("No passphrase configured: cannot download or decrypt encrypted backups.")
    ...
    def _decode_backup_payload(self, key: str, payload: bytes) -> dict[str, object]:
        ...
        if key.endswith(".enc"):
            passphrase = _clean(config.get_backup_settings().get("passphrase"))
            if not passphrase:
                raise BackupError("No passphrase configured: cannot view encrypted backups.")
    ...
    def _decode_archive_detail(self, payload: bytes) -> dict[str, object]:
        ...
        try:
            ...
            raise BackupError("Failed to parse backup archive; it may be corrupted") from exc
